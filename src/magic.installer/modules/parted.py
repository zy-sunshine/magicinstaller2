...
##### Utility
        ...
        def close_dialog(self, operid, data):
            data.topwin.destroy()
...
##### Gather info
        def refresh_disklabel_label(self, operid, data):
            "Callback of op 'get_disk_type'"
            (self.disklabel, self.support_partition_name) = self.tm.results[operid]
        ...
        def fill_global_and_check(self):
            ...
            while iter:
                ...
                if format_or_not == 'true':
                    ...
                    not_touched = 'false'
                all_part_infor[self.devfn].append((partnum,
                                                   parttype,
                                                   partflags,
                                                   start,
                                                   end,
                ...
                if not_touched == 'true' and \
                    ...
                    all_orig_part.append((self.orig_partitions[start],
                                          filesystem,
                ...
                if mountpoint == '/':
                    ...
                    root_device = '%s%d' % (self.devfn, partnum)
                if mountpoint == '/boot':
                    boot_device = '%s%d' % (self.devfn, partnum)
                if fstype_map.has_key(filesystem):
                    minsize = fstype_map[filesystem][2]
                    if minsize != -1:
                        if end - start + 1 < minsize * 2048:
                            ...
                            return  errmsg
                    maxsize = fstype_map[filesystem][3]
                    if maxsize != -1:
                ...
                if filesystem == 'linux-swap' and mountpoint == 'USE':
                    ...
                    if not swap_device or swap_device[2] < end - start + 1:
                        swap_device = [self.devfn, partnum, end - start + 1]
                iter = model.iter_next(iter)
            return  None

        def got_all_partitions(self, operid, data):
            "Callback of op 'get_all_partitions'"
            self.tm.add_action(None, self.refresh_disklabel_label, None,
                               'get_disk_type', self.devfn)
            self.partlist = self.tm.results[operid]
        ...
        def reload_all_partitions(self, operid, data):
...
##### Partition create and edit
        ...
        def partition_added(self, operid, data):
            (start, errstr) = self.tm.results[operid]
...
##### Enter & Leave Check
    def enter(self):
        if self.probeall_status != OP_STATUS_DONE:
    ...
    def leave(self):
        ...
        for hdobj in self.hdobj_list:
            ...
            if errmsg:
                magicpopup.magicmsgbox(None,
                                       ...
                                       magicpopup.magicpopup.MB_OK)
                return 0
        dolog('all_part_infor = %s\n' % (all_part_infor, ))
        dolog('all_orig_part = %s\n' % (all_orig_part, ))
        ...
        mountpoints = {}
        for api in all_part_infor.keys():
            for p in all_part_infor[api]:
                if p[7] == '' or p[7] == 'USE':
                    continue
        ...
        if bt_instpos == 'boot' and \
            ...
            warnmsg = warnmsg % (boot_device, get_devinfo(boot_device).fstype)
            magicpopup.magicmsgbox(None, warnmsg,
                                   magicpopup.magicmsgbox.MB_WARNING,
                                   magicpopup.magicpopup.MB_OK)
            self.set_data(self.rootobj.values, 'bootloader.instpos', 'mbr')

        self.set_data(self.rootobj.values, 'bootloader.boot_device', boot_device)
...
##### Check Dir Size
    ...
    def check_dir_size_ignore_clicked(self, widget, dlg):
        ...
        self.rootobj.name_widget('btnnext').clicked()

##### Probe Result
    def got_pkgarr_probe_result(self, operid, data):
        global pkgarr_probe_status
    ...
    def got_win_probe_result(self, operid, data):
        ...
        win_probe_result = self.rootobj.tm.results[operid]
        win_probe_status = OP_STATUS_DONE
        dolog('win_probe_result: %s\n' % str(win_probe_result))
        ...
        if win_probe_result:
            ...
            win_probe_result.sort(win_part_cmp)
            win_device = win_probe_result[0][0]
        else:
            win_device = ''
...
##### Misc
    def btnhelp_clicked(self, widget, data):
        ...
        # file.
        self.default_btnhelp_clicked(widget, data, _('helptext/parted.help.en.txt'))
            ...
            def action(self):
                ...
                for (i, part) in enumerate(self.autoparts):
                    if i < 3:
                        ...
                        (rc, errmsg) = self.tm.results[self.operid]
                        if rc < 0:
                            errtxt = _(errmsg)
                            magicpopup.magicmsgbox(None, errtxt,
                                                   magicpopup.magicmsgbox.MB_INFO,
                    ...
                    yield self.tm.add_action(None, self, None,
                                             ...
                                             start_sect, end_sect)
                    (real_start_sect, errmsg) = self.tm.results[self.operid]
