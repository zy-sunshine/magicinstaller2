...
def resetRpmDb():
    for rpmfile in glob.glob("%s/var/lib/rpm/__db.*" % CF.D.TGTSYS_ROOT):
        try:
            os.unlink(rpmfile)
        except OSError as e:
...
def copy_to_sysimage(source):
    ...
    target = CF.D.TGTSYS_ROOT + source
...
def dracut_eject(device):
    ...
    try:
        if not os.path.exists(CF.D.DRACUT_SHUTDOWN_EJECT):
            f = open(CF.D.DRACUT_SHUTDOWN_EJECT, "w")
            f.write("#!/bin/sh\n")
            f.write("# Created by Anaconda\n")
        else:
            f = open(CF.D.DRACUT_SHUTDOWN_EJECT, "a")

        f.write("eject %s\n" % (device,))
        f.close()
        os.chmod(CF.D.DRACUT_SHUTDOWN_EJECT, 0755)
